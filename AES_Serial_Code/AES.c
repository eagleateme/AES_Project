...
static const uint8_t Mix_Mat[4][4] = {{0x02,0x03,0x01,0x01},{0x01,0x02,0x03,0x01},{0x01,0x01,0x02,0x03},{0x03,0x01,0x01,0x02}};
...
void Copy__state_str()
{
	int i = 0;
		...
		for(int j = 0 ;  j < 4; j++)
		{
			for(int k = 0 ;  k < 4; k++)
			{
				state_t[k][j] = plain_arr[16*counter+i];
				i++;
			}
		}
}
...
void Generate_Plain_Text()
{
	for(int count = 0 ; count < 1500 ; count++)
	{
		...
		for(int i = 15 ; i >= 0 ; i--)
		{
			int temp = plain_arr[16*count+i]+counter_1;

				if(temp == 256)//Not Sure if this would work like this but temp > 256 (to Generalize the Adding)
				{
					plain_arr[(16*(count+1))+i] = 0x00; // \\Same Issue as the Previous Comment States\\ = temp%256;
					counter_1 = 0x01; // = temp/256
				}
				else{
					plain_arr[(16*(count+1))+i] = plain_arr[(16*(count))+i]+counter_1;
					counter_1 = 0x00;
				}
		}
...
void Copy_Plain()
{
	...
	for(int j = 0 ;  j < 4; j++)
	{
		for(int k = 0 ;  k < 4; k++)
		{
			plain_arr[16*counter+i] = state_t[k][j];
			i++;
		}
	}
}
...
int main()
{
	...
	uint8_t* input_arr = (char*)calloc(24000,sizeof(char));

	FILE* f1 = fopen("plain_text.txt","r");

	fread(input_arr,sizeof(char),24000,f1);
	...
	while(counter < 10)
	{
		Copy__state_str();//Copy To The Temp for Multiplication
		AddRoundKey(0);
		for(int i = 1; i <= 10 ; i++)
		{
			SubBytes();
